...
describe("Tests For importExport utils", () => {
  ...
  describe("validateImported", () => {
    ...
    it("should mark importExists as true when the resource exists in the db", async () => {
      ...
      expect(Category.exists).toHaveBeenCalledWith("Category 1");
      expect(Bookmark.exists).toHaveBeenCalledWith("bookmark1");
      expect(Bookmark.exists).toHaveBeenCalledWith("bookmark2");
      expect(Tag.exists).toHaveBeenCalledWith("Tag 1");
      expect(Tag.exists).toHaveBeenCalledWith("Tag 2");
      expect(Tag.exists).toHaveBeenCalledWith("Tag 3");
    });
  ...
  describe("isImportData", () => {
    it("should return true for valid array of categories", () => {
      ...
      categories[0].bookmarks = bookmarks;
      expect(importExport.isImportData(categories)).toBe(true);
      categories[0].bookmarks[0].url = "invalid-url";
      expect(importExport.isImportData(categories)).toBe(false);

      // Tags validation
      categories[0].tags = tags;
      // reset the invalid url
      categories[0].bookmarks[0].url = "http://example.com";
      expect(importExport.isImportData(categories)).toBe(true);
      categories[0].bookmarks[0].tags = invalidTags;
      expect(importExport.isImportData(categories)).toBe(false);
    });
  ...
  describe("markBookmarkExists", () => {
    ...
    it("should handle mixed existence states for bookmarks and tags", async () => {
      ...
      vi.spyOn(Tag, 'exists').mockImplementation(async (name) => {
        if (name === "Tag 1") return {} as ITag;
        return null;
      });
