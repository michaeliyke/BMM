#!/usr/bin/python3
"""Handles posting a new tag to a category,
and getting all tags of a category
...
@app_views.route('/categories/<category_id>/tags', methods=['GET', 'POST'],
                 strict_slashes=False)
def get_all_category_tags_or_create(category_id):
    """
    Retrieves the list of all Tag objects of a Category OR
    creates(or adds) a new Tag object for a Category
    """
    if request.method == 'GET':
        try:
            ...
            tags = [tag.to_dict() for tag in category.tags]
...
# curl -X POST -H "Content-Type: application/json" -d '{"name":"new tag"}' 0/bmm/api/categories/1/tags
# curl -X GET http://localhost/bmm/api/categories/1/tags
